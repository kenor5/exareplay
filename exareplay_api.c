...
void exareplay_free(exareplay_t *ctx) {
    safe_free(ctx->opts);
    safe_free(ctx);
}

void device_init(exareplay_t *ctx) {
...
void device_close(exareplay_t *ctx) {
...
int exareplay_parse_args(exareplay_t *ctx, int argc, char* argv[]) {
    int c;
    if (argc < 2)
        goto usage_err;
...
void exareplay_replay(exareplay_t *ctx) {
