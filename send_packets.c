...
load_pcap(exareplay_t *ctx)
{
    ...
    while (pcap_next(cap, pkt_header) != NULL) {
        ...
        ctx->pcap_info->time_interval[idx] =
                SEC_TO_NANOSEC(pkt_header->ts.tv_sec) + (long)pkt_header->ts.tv_usec - pre_time;
        ctx->pcap_info->time_interval[idx] = (long)max(ctx->pcap_info->time_interval[idx] - time_delta, 0);
        ctx->pcap_info->time_interval[idx] =
                (long)(((ctx->pcap_info->time_interval[idx] > burst_interval_min &&
                         ctx->pcap_info->time_interval[idx] < burst_interval_max)
