...
// 打印指定线程的绑核id
void
print_cpu_affinity_id(int core_nums, pthread_t tid, cpu_set_t *cpu_set)
{
    int i = 0;
...
// 获取指定线程的绑核id
int
frank_phtread_cpu_affinity_get(pthread_t tid, cpu_set_t *cpu_set)
{
    CPU_ZERO(cpu_set);
...
// 指定线程绑核
int
frank_pthread_single_cpu_affinity_set(int core_id, pthread_t tid)
{
    cpu_set_t mask;
